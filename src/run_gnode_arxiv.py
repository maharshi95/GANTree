...
def relabel_samples(node, dl):
    candidate_data_dict = {
        split: node.filter_x(dl.data[split], prob_threshold=0.5)
...
def train_phase_1(n_iterations):
    root.fit_gmm(x_seed)
    visualize_plots(iter_no=0, node=root, x_batch=x_seed, labels=l_seed, tag='x_clf_plots')

    with tqdm(total=n_iterations) as pbar:
        for iter_no in range(n_iterations):
            root.trainer.iter_no = iter_no
            ...
            z_batch, x_recon, x_recon_loss, x_clf_loss, loss = root.step_train_x_clf(x_clf_train_batch)

            root.trainer.writer['train'].add_scalar('x_clf_loss', x_clf_loss, iter_no)
            root.trainer.writer['train'].add_scalar('x_recon_loss', x_recon_loss, iter_no)
            root.trainer.writer['train'].add_scalar('loss', loss, iter_no)

            root.fit_gmm(x_seed)
            if i < 10 or i % 10 == 0:
                visualize_plots(iter_no=i, node=root, x_batch=x_seed, labels=l_seed, tag='x_clf_plots')
            pbar.update(n=1)
...
def train_node(root_node):
    child_nodes = tree.split_node(root_node, fixed=False)
    ...
    train_phase_1(x_clf_iters)
    relabel_samples()
    train_phase_2(gan_iters)
