...
default_args_str = '-hp hyperparams/digit_mnist_64.py -en mnist_mode_split_twoPhase -t'
        ...
        with tqdm(total=n_iterations) as pbar:
            for iter_no in range(n_iterations):
                ...
                if current_iter_no % 20 == 0:
                    simcrossdist = node.update_child_params(x_seed, Z= dl_set[0].random_batch('train', 1024), max_iter=5, applyPCA= False)
            
                    node.trainer.writer['train'].add_scalar('simcrossdist', simcrossdist, current_iter_no)
                ...
                if meanDistance < 15:
                    z_batch, x_recon, x_recon_loss, x_clf_loss, loss, preds, time_taken, mu_hinge_loss, x_clf_cross_loss = node.step_train_x_clf(x_clf_train_batch, start_no, end_no, w1 = 1.0, w2 = 0.0, w3 = 10.0, w4 = 10.0, frozenLabels = False)
                else:
                    z_batch, x_recon, x_recon_loss, x_clf_loss, loss, preds, time_taken, mu_hinge_loss, x_clf_cross_loss = node.step_train_x_clf(x_clf_train_batch, start_no, end_no, w1 = 10.0, w2 = 1.0, w3 = 0.0, w4 = 10.0, frozenLabels = True)
...
def train_node(node, x_clf_iters=200, gan_iters=10000):
    ...
    child_nodes = tree.split_node(node, x_batch = dl_set[0].train_data(), fixed=False, applyPCA = False)
