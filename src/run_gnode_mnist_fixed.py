...
default_args_str = '-hp hyperparams/digit_mnist_64.py -en mnist_mode_split_fixed -t'
...
def train_phase_1(node, epochs):
    ...
    for j in range(epochs):
        ...
        node = GNode.loadwithkmeans('best_root_phase1_mnistdc_'+ str(j) +'.pickle', node)
        child0 = GNode.loadwithkmeans('best_child0_phase1_mnistdc_'+ str(j) + '.pickle', node.get_child(0))
        child1 = GNode.loadwithkmeans('best_child1_phase1_mnistdc_'+ str(j) + '.pickle', node.get_child(1))
        ...
        with tqdm(total=n_iterations) as pbar:
            for iter_no in range(n_iterations):
                ...
                if (j >= 0) and (current_iter_no % 200 == 0):
                    ...
                    plt.bar(r1, percentAbove_ch0, width = barWidth, color = 'red', edgecolor = 'black', capsize=7)
                    plt.bar(r2, percentAbove_ch1, width = barWidth, color = 'blue', edgecolor = 'black', capsize=7)
                ...
                if (iter_no < (n_iterations - 1)) and j < phase1_epochs:
                    ...
                    if length_assi == 0 or length_unassi == 0:
                        ...
                        training_list.append(random.sample(unassigned_index, min(length_assi, length_unassi)))
                    else:

                        start_no_assi = (iter_no * batchSize / 2) % length_assi
                        end_no_assi = (start_no_assi + batchSize/2) % length_assi

                        if end_no_assi < start_no_assi:
                            training_list = [i for i in assigned_index[start_no_assi:]]
                            training_list.append([i for i in assigned_index[:end_no_assi]])
                        else:
                            training_list = [i for i in assigned_index[start_no_assi:end_no_assi]]

                        start_no_unassi = (iter_no * batchSize/2) % length_unassi
                        end_no_unassi = (start_no_unassi + batchSize/2) % length_unassi

                        if end_no_unassi < start_no_unassi:
                            training_list.append([i for i in unassigned_index[start_no_unassi:]])
                            training_list.append([i for i in unassigned_index[:end_no_unassi]])
                        else:
                            training_list.append([i for i in unassigned_index[start_no_unassi:end_no_unassi]])
...
def get_recons_data(node, x_batch, l_seed):
    ...
    with tr.no_grad():
        ...
        np.savez('x_recon_post_child0_fixed_0', recon_ch0 = x_recon_post_child0, l_seed_ch0 = l_seed_ch0)
        np.savez('x_recon_post_child1_fixed_0', recon_ch1 = x_recon_post_child1, l_seed_ch1 = l_seed_ch1)
