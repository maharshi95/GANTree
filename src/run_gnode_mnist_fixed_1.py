...
default_args_str = '-hp hyperparams/digit_mnist_64.py -en mnist_mode_split_fixed_1 -t'
...
def get_plot_data(iter_no, node, x_batch, labels, threshold, pca2rand = None, pca = False):
    ...
    pred_post = node.gmm_predict_test(z_batch_post)
    ...
    if pca:
        ...
        print(pca2rand.explained_variance_)
...
def visualize_plots(iter_no, node, x_batch, labels, tag, threshold, pca2rand = None):
    plot_data, pca2rand = get_plot_data(iter_no, node, x_batch, labels, pca2rand = pca2rand, pca = True, threshold = threshold)
    generate_plots(plot_data, iter_no, tag, node)
    return pca2rand
...
def train_phase_1(node, epochs):
    ...
    for j in range(1):
        ...
        cross_distance_matrix = np.zeros((10,10), dtype = float)
        max_distance_matrix = np.zeros((10,10), dtype = float)

        for i in range(10):
            for k in range(i, 10):
                focusClass = Z[np.where(train_data_labels == i)]
                crossClass = Z[np.where(train_data_labels == k)]

                # print(len(focusClass))
                # print(len(crossClass))

                # print(type(focusClass[0]))
                # print(type(crossClass[0]))

                # d = np.linalg.norm(focusClass[0].cpu().numpy() - crossClass[0].cpu().numpy())
                # print(d)
                ...
                for w in range(len(focusClass)):
                    d = np.linalg.norm(focusClass[w] - crossClass, axis = 1)
                    # print(d.shape)
                    ...
                    # for z in range(len(crossClass)):
                    #     d = np.linalg.norm(focusClass[w] - crossClass[z])
                ...
                # cross_distance_matrix[i][k] = avg_distance
                cross_distance_matrix[i][k] = distance / count

                print(cross_distance_matrix[i][k])

                max_distance_matrix[i][k] = max_d

                np.savez("cross_distance", a = cross_distance_matrix)
                np.savez("max_distance_matrix", a = max_distance_matrix)

        print(cross_distance_matrix)
        ...
        with tqdm(total=n_iterations) as pbar:
            for iter_no in range(n_iterations):
                ...
                if (j >= 0) and (current_iter_no % 200 == 0):
                    # free some labels
                    if j == 5 and current_iter_no == 5000:
                ...
                z_batch, x_recon, preds, x_clf_loss_assigned, x_assigned_recon_loss, loss_assigned, x_clf_loss_unassigned, x_unassigned_recon_loss, loss_unassigned, x_clf_cross_loss, loss_recon = node.step_train_x_clf(x_clf_train_batch, start_no, end_no, w1 = 1.0, w2 = 1.0, w3 = 0.0, w4 = 0.0, with_PCA = True, threshold = threshold)
...
def get_recons_data(node, x_batch, l_seed):
    ...
    with tr.no_grad():
        ...
        np.savez('x_recon_post_child0_fixed_1', recon_ch0 = x_recon_post_child0, l_seed_ch0 = l_seed_ch0)
        np.savez('x_recon_post_child1_fixed_1', recon_ch1 = x_recon_post_child1, l_seed_ch1 = l_seed_ch1)
...
try:
    ...
    train_node(root, x_clf_iters=12, gan_iters=20000)  # , min_gan_iters=5000, x_clf_lim=0.00001, x_recon_limit=0.004)
