...
default_args_str = '-hp hyperparams/digit_mnist_64.py -en mnist_mode_split_fixed_6 -t'
...
def visualize_plots_phase1part2(iter_no, node, x_batch, labels, tag, threshold, pca2rand = None):
...
""" cluster the points after separating the means according to a fixed threshold (phase 1 part 1)
    phase 1 -- mode separation 
        |
        |-- part 1 -- separation of mean
        |
...
def train_phase_1_part2(node, threshold = 2.5, part2_epochs = 7):
    ...
    # node = GNode.loadwithkmeans('best_root_phase1_mnistdc_'+ str(part2_epochs) +'.pickle', node)
    # child0 = GNode.loadwithkmeans('best_child0_phase1_mnistdc_'+ str(part2_epochs) + '.pickle', node.get_child(0))
    # child1 = GNode.loadwithkmeans('best_child1_phase1_mnistdc_'+ str(part2_epochs) + '.pickle', node.get_child(1))
    # node.child_nodes = {1: child0, 2: child1}


    """ train part 2 of mode separation algorithm (phase 1) """

    for j in range(0, part2_epochs):

        print("epoch number: " + str(j))


        """ reassign labels at the start of reassignment_epoch """
        ...
        with tqdm(total=n_iterations) as pbar:
            for iter_no in range(n_iterations):
                ...
                if length_assi == 0 or length_unassi == 0:
                    ...
                    """ if no assigned or unassigned labels, take a normal batch """
                    ...
                    while(len(training_list) < batchSize):
                        training_list.extend(random.sample(assigned_index, min(length_unassi, length_assi)))
                        training_list.extend(random.sample(unassigned_index, min(length_assi, length_unassi)))
                ...
                """ visualize the test embeddings
                    visualize the ratio of assigned labels in cluster 0 and cluster 1 
                    every 600 iterations """

                if current_iter_no % 600 == 0 and current_iter_no != 0:
                    visualize_plots_phase1part2(iter_no=current_iter_no, node=node, x_batch=x_seed, labels=l_seed, tag='x_clf_plots', pca2rand = pca2rand, threshold = threshold)


""" separate the two means to a certain threshold (phase 1 part 1)
    phase 1 -- mode separation 
        |
        |-- part 1 -- separation of mean
        |
...
def get_recons_data(node, x_batch, l_seed):
    ...
    with tr.no_grad():
        ...
        np.savez('x_recon_post_child0_fixed_6', recon_ch0 = x_recon_post_child0, l_seed_ch0 = l_seed_ch0)
        np.savez('x_recon_post_child1_fixed_6', recon_ch1 = x_recon_post_child1, l_seed_ch1 = l_seed_ch1)
...
root = GNode.loadwithkmeans('best_root_phase1_mnistdc_'+ str(part2_epochs) +'.pickle', root)
...
try:
    ...
    train_node(root, x_clf_iters=1300, part2_epochs = 14, gan_iters=20000)  # , min_gan_iters=5000, x_clf_lim=0.00001, x_recon_limit=0.004)
