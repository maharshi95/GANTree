...
class ImgEncoder(BaseModel):
    def __init__(self, z_size, out_scale=4.0):
        ...
        # 3  * 64 x 64
        self.conv1 = ConvBlock(3, 32, kernel_size=5, stride=1, padding='same')
        # 32 * 64 x 64
        ...
        # 256 * 8 x 8
        self.conv5 = ConvBlock(256, 256, kernel_size=5, stride=2, padding='same')
        # 256 * 4 x 4
...
class ImgDecoder(BaseModel):
    def __init__(self, z_size, out_scale=4.0):
        ...
        self.fc2 = nn.Linear(200, 4 * 4 * 256)

        # 256 * 4 * 4
        self.tconv1 = UpConvBlock(256, 128, kernel_size=5, stride=2, output_padding=(1, 1))
        # 128 * 8 * 8
        self.tconv2 = UpConvBlock(128, 64, kernel_size=5, stride=2, output_padding=(1, 1))
        # 64 * 16 * 16
        self.tconv3 = UpConvBlock(64, 32, kernel_size=5, stride=2, output_padding=(1, 1))
        # 32 * 32 * 32
        self.tconv4 = UpConvBlock(32, 3, kernel_size=5, stride=2, output_padding=(1, 1))
        # 3 * 64 * 64
        self.init_params()
