...
class GanTree(BaseModel):
    ...
    def split_node(self, parent, x_batch=None, fixed=True, applyPCA = True):
        # type: (GNode, np.ndarray, bool) -> list[GNode]
        x_batch = self.x_batch if x_batch is None else x_batch

        child_nodes = GNodeUtils.split_node(parent, self.n_child, x_batch, base_id=len(self.nodes), fixed=fixed, applyPCA = applyPCA)
    ...
    def load_children(self, parent, phase1_epochs):
        child_nodes = GNodeUtils.load_children(parent, self.n_child, phase1_epochs, base_id = len(self.nodes))
