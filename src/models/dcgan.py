...
class Model(BaseModel):
    ...
    def _define_placeholders(self): # change
        ...
        self.ph_plot_img = tf.placeholder(tf.float32, [None, None, None, 4])   # ask

    def _define_network_graph(self): # ask
        with tf.variable_scope(self.model_scope, reuse=tf.AUTO_REUSE):
            # X - Z - X Iteration
            self.x_real = self.ph_X
            self.out_real, self.logits_real,self.z_real = encoder(self.x_real) # ask
            self.x_recon = decoder(self.z_real)

            # Z - X - Z Iteration
            self.z_rand = self.ph_Z
            self.x_fake = decoder(self.z_rand)
            self.out_fake, self.logits_fake,self.z_recon = encoder(self.x_fake)  # ask
    ...
    def _define_losses(self): # ask
        ...
        self.disc_loss_real = tf.losses.softmax_cross_entropy(real_labels, self.logits_real) *(1 - smooth)
    ...
    def _define_metrics(self):
        self.disc_real_preds = tf.cast(tf.greater(self.logits_real,0),H.dtype)
        self.disc_fake_preds = tf.cast(tf.greater(self.logits_fake,0),H.dtype)
